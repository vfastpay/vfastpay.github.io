...
## 通知地址
该链接是通过基础代收接口中的请求参数`notify_url`来设置的，请确保回调URL是外部可正常访问。

## 通知方式
POST

## 通知规则
代收完成后，我方会把相关结果信息发送给对接方，对接方需要接收处理该消息，并返回应答。如果应答不符合规范，认为通知失败，会重新发起通知，最大通知次数为5次，五次通知时间分别为：10s、20s、40s、80s、160s。
